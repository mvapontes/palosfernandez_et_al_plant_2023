...
# ChIP-Seq and RNA-Seq data per base coverage of selected genes
...
genes <- as.vector(
  unlist(
    read.csv(
      here("input", "selectedGenes_Manuscript.txt"),
      header = TRUE)
    )
  )
...
genome_sum <- genome_data %>%
  ...
  mutate(name = case_when(Chrm.ID != "unplaced" ~ Chrm.ID, 
                          TRUE ~ RefSeq.Accn)
         ) # add new variable name, if Chrm.ID is not unplaced then used Chrm.ID otherwise RefSeq.Accn
...
cyto<- genome_data %>%
  ...
  dplyr::select(RefSeq.Accn, Sequence.Name, Chrm.ID,Sequence.Length, start, end, status) %>% 
  ...
  # to not combine unplaced contigs in one line adjust start and end positions accordingly
  ...
  # add colors to areas in chromosomes
  ...
  filter(RefSeq.Accn != "na") %>% # remove those without refseqid, ex. mitochondrial chromosome 
  mutate(name = case_when(Chrm.ID != "unplaced" ~ Chrm.ID,
                          TRUE ~ RefSeq.Accn))

# GRanges vector with cytobands information
...
# From gff to database txdb with transcript annotations
...
for (n in 1:length(genes)){
  message("GOT a new gene")
  
  # get info for out gene of interest
  
  f_genes <- feature.genes[feature.genes$Name == genes[n]] # get gene info
  ...
  f_genes_df.start <- f_genes_df$start -1000 # left position + 1000bp window
  f_genes_df.end <- f_genes_df$end + 1000 # right position + 1000 bp window
  ...
  # check if selected genes fit in the plotted region
  ...
  # update GRanges plotting region negative strand
  kp$plot.region<- GRanges(seqnames = Rle(kp_df$seqnames),
                           ranges=IRanges(start = kp_df$start,
                                          end = kp_df$end),
                           strand=Rle("-"))
  ...
  tiff(here("plots", paste0(genes[n], "_methylation_nogenename.tiff")),
       width = 6000, 
       height = 3000,
       res = 300,
       compression = "none")
  ...
  suppressWarnings(kpPlotGenes(kp, 
                               ...
                               r0 = 0, 
                               r1 = 0.15, 
                               add.gene.names = FALSE, # TRUE to add gene name to plot
  ...
  kpAddLabels(kp, 
              labels = expression(italic('mac1')*Delta*' -Cu'), 
              r0 = 0.8, 
              r1 = 0.6, 
              label.margin = 0.01, 
              data.panel = 1, 
  ...
  # ChIP-Seq data (wig extension file)
